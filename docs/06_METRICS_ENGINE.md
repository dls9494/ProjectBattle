...
MissionSnapshot is a read-only Domain Model.

It represents the complete current state of the active mission.

MissionSnapshot is generated by the Metrics Engine.

MissionSnapshot MUST NOT be persisted.

MissionSnapshot MUST be regenerated whenever underlying data changes.

---

## Responsibilities

MissionSnapshot MUST:

- Aggregate mission data.
- Aggregate body metrics.
- Aggregate daily metrics.
- Aggregate projections.
- Aggregate intelligence inputs.
- Be optimized for UI rendering.

MissionSnapshot MUST NOT:
...
MissionSnapshot is generated from:
...
|-------|------|-------------|
| missionId | UUID | Reference to mission |
| missionName | String | Human-readable name |
| missionStatus | Enum | Active, Paused, Completed, Archived |
| startWeightKg | Double | Weight at mission start |
| currentWeightKg | Double | Latest official weight |
| targetWeightKg | Double | Target weight |
| remainingWeightKg | Double | Current - Target |
| weightProgressPercent | Double | % of target achieved |
| bmr | Double | Basal Metabolic Rate |
| maintenanceCalories | Double | Daily maintenance |
| dailyDeficit | Double | Today's deficit |
| totalDeficit | Double | Cumulative deficit |
| remainingCalories | Double | Calories remaining |
| requiredDailyDeficit | Double | Needed per day |
| projectionDate | Date | Estimated completion |
| confidenceScore | Integer | 0–100 |
| dataReliabilityScore | Integer | 0–100 |
| today | DailySnapshot | Today's summary |
| weekToDate | WeeklySnapshot | Weekly summary |
